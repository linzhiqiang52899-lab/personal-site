...
const CONTENT_DIRS = [
  ...
  'C:\\Users\\pqluc\\.qclaw\\media\\inbound\\content'
];
...
async function startServer() {
  ...
  // 监听所有内容目录的文件变化
  const watchers = CONTENT_DIRS.map(dir => {
    const watcher = chokidar.watch(dir, {
      ignored: /(^|[\\/\\])\../,
      persistent: true,
      ignoreInitial: true
    });
    watcher.on('all', () => scanContentDir());
    return watcher;
  });
  ...
  app.listen(PORT, () => {
    ...
    console.log(`📂 内容目录:`);
    CONTENT_DIRS.forEach(dir => console.log(`   - ${dir}`));
    console.log(`\n💡 提示: 将 .md、.html、.pdf 或 .txt 文件放入以上目录即可自动访问\n`);
  });
}
